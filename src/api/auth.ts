...
export const authAPI = {
  ...
  login: async (email: string, password: string) => {
    const response = await axiosInstance.post<TokenResponseDto>('/api/auth/login', {
      email,
      password,
    });
  ...
  signup: async (data: {
    ...
    const response = await axiosInstance.post<TokenResponseDto>('/api/auth/signup', data);
    return response.data;
  },
  ...
  resendVerificationCode: async (email: string) => {
    const response = await axiosInstance.post<string>('/api/auth/email/resend', { email });
    return response.data;
  },
  ...
  refresh: async () => {
    ...
    const response = await axiosInstance.post<TokenResponseDto>(`/api/auth/refresh`, { refreshToken });
    return response.data;
  },
};
